...
uses
  ...
  DOM, XMLRead, XMLWrite, typinfo, lhttp, lNet, CsvDocument, fpjson;
...
begin
  ...
  begin
    ...
    begin
      ch2 := url[x+1];
      subStr := Copy(url, x+2, 10);
      position := pos(';', subStr);
      if (ch2 <> '#') or (position = 0) then
      begin
        //Append other chars
        Buff := Buff + ch;
      end
        ...
        begin
        //Get value
        sVal := Copy(url, x + 2, position -1);
        //Convert sval to int then to char
        Buff := Buff + char(StrToInt(sVal));
        //Inc counter by  2 + position -1 = position + 1
        Inc(x, position +1);
        end;
    end;
    //Inc counter
...
var
  ...
  buffer : string;
  TransString , TransString2 : string;
  ...
  try
    ...
    begin // status is OK
      response := Copy(HTTPBuffer, pos('"translatedText":', HTTPBuffer) +
        17, length(HTTPBuffer));
      TransString := Copy(response, 0, pos(',"match"', response) - 1);
      TransString2 := DecodeUnicodeEscapes(TransString);
      TranslatedString := DecodeHtml(TransString2);
    end
    else
...
begin
  ...
  begin
    ...
    begin
      if LowerCase(sLang) <> MemDtsLanguages.FieldByName('Language').AsString then
      begin
        sLang := LowerCase(MemDtsLanguages.FieldByName('Language').AsString);
        if Length(sLang)>0 then
           sLang:= UpperCase(sLang[1])+Copy(sLang,2,Length(sLang)-1 );
        cbOriginalLang.Items.Add(sLang);
        cbTranslatedLang.Items.Add(sLang);
      end;
      MemDtsLanguages.Next;
    end;
    iOrig := cbOriginalLang.Items.IndexOf('en');
    iTrans := cbTranslatedLang.Items.IndexOf('fr');
    if iOrig <> -1 then
...
begin
  try
    try
      ...
      HTTPClient.Host := 'mymemory.translated.net'; //'ajax.googleapis.com';
//      HTTPClient.URI := '/ajax/services/language/translate?v=1.0&q=' +
//        URLEncode(Source) + '&langpair=' + langpair;
//        https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=fr&dt=t&q="sample text"
//        http://mymemory.translated.net/api/get?q=Hello World!&langpair=en|it
//      HTTPClient.URI := 'translate_a/single?client=gtx&sl=' + LeftStr(langpair,2) + '&tl=' +  RightStr(langpair,2) + '&dt=t&q=' + URLEncode(Source);
      HTTPClient.URI := '/api/get?q=' +
                     URLEncode(Source) + '&langpair=' + LeftStr(langpair,2) + '|' + RightStr(langpair,2);
...
begin
  ...
  begin
    ...
    try
      ...
      // if exists SystemUI.apk, preload it
      if FindMatchStr(chklstApk.Items, 'SystemUI.apk') >= 0 then
      begin
        meLog.Lines.Add(
          '--Preloading SystemUI.apk------------------------------');
        RunAppInMemo(AppendPathDelim(ApplicationDirectory + csScriptsDir) +
          'java' + csScriptExt + ' -jar "' + AppendPathDelim(
          ApplicationDirectory + csOtherDir) + 'apktool.jar" if "' +
          AppendPathDelim(ApplicationDirectory + csModdingDir) + 'SystemUI.apk"',
          meLog);
      end;
              ...
              begin
                ...
                begin
                  ...
                  iPosLang :=
                    ...
                    AppendPathDelim(slDir[i]) + 'res') + 'values-' +
                    LowerCase(cbTranslatedLang.Text));
                  if iPosLang >= 0 then
                  begin
                    sXmlFile :=
...
begin
  ...
  if FileExistsUTF8(ApplicationDirectory + 'Dictionary' + cbOriginalLang.Text +
    cbTranslatedLang.Text + '.csv') then
  begin
    try
      FileStream := TFileStream.Create( ApplicationDirectory + 'Dictionary' +
        cbOriginalLang.Text + cbTranslatedLang.Text + '.csv',
    ...
    begin
      try
        ...
        if trim(MemDtsTranslations.FieldByName('TranslatedText').AsWideString)
          = '' then
          sOriginalText := MemDtsTranslations.FieldByName('OriginalText').AsWideString;
          if (  LeftStr(sOriginalText,1) = '"') and  (RightStr(sOriginalText,1) = '"') then
          begin
              sOriginalText := Copy(sOriginalText, 2 , length(sOriginalText) - 2 );
          end;
          if GoogleTranslate(sOriginalText, LowerCase(cbOriginalLang.Text) + '|' +
            LowerCase(cbTranslatedLang.Text), sValue) then
          begin
            MemDtsTranslations.Edit;
            MemDtsTranslations.FieldByName('TranslatedText').AsWideString :=
  ...
  try
    ...
    begin
      ...
      if (FileExistsUTF8(AppendPathDelim(ApplicationDirectory) +
        'Dictionary' + cbOriginalLang.Text + cbTranslatedLang.Text + '.csv')) and
...
begin
  ...
  try
    ...
    try
      for i := 0 to slDir.Count - 1 do
      begin
        sLangTranslated := LowerCase(cbTranslatedLang.Text);
        if chkStringFromValues.Checked then
          sLangSource := ''
        else
          sLangSource := '-' + LowerCase(cbOriginalLang.Text);
...
begin
  ...
  begin
    sFile := AppendPathDelim(ApplicationDirectory) + 'Dictionary' +
      cbOriginalLang.Text + cbTranslatedLang.Text + '.csv';
